...
def resource_path(relative_path):
    """
    Get absolute path to resource, to ensure functionality for both dev and for PyInstaller
    (This is for the AppImage build which allows my program to function as a desktop application)
    """
    try:
        # Running as a PyInstaller bundle
        # If this is the case, sys._MEIPASS will be defined
...
class ReactionGame(PySide6.QtWidgets.QMainWindow):
    ...
    def initUI(self):
        ...
        layout.addStretch(
            int((0.5 - INSTRUCTION_BOX_HEIGHT / 2.0) * STRETCH_PRECISION)
    ...
    def animate_trophy(self):
        ...
        if self.trophy_animation_group is not None:
            ...
            self.trophy_animation_group = None  # Clear reference

        # Create a new sequential group for the new animation
        animation_sequence = PySide6.QtCore.QSequentialAnimationGroup(self)

        # --- Calculate Size ---
        top_bar_height = self.top_bar.height() if self.top_bar.isVisible() else 0
        # The top bar *should* be visible, but check anyway for future proofing
        instruction_box_y = self.instruction_box.y()
        available_height = instruction_box_y - top_bar_height
        padding_factor = 1.0 - TROPHY_PADDING
        max_height = int(available_height * padding_factor)
        max_width = int(self.width() * padding_factor)

        target_size = min(max_height, max_width)

        # --- Animate ---
        grow_animation = PySide6.QtCore.QPropertyAnimation(self.trophy, b"minimumSize")
        ...
        if self.reaction_time < self.best_time:
            ...
            if not self.top_bar.isVisible():
                ...
                new_stretch = layout.stretch(1) - int(
                    TOP_BAR_HEIGHT * STRETCH_PRECISION
                )
